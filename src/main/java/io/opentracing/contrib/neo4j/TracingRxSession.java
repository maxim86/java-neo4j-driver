...
public class TracingRxSession implements RxSession {
  ...
  public Publisher<RxTransaction> beginTransaction() {
    return rxSession.beginTransaction();
  }

  @Override
  public Publisher<RxTransaction> beginTransaction(TransactionConfig transactionConfig) {
    return rxSession.beginTransaction(transactionConfig);
  }
  ...
  public <T> Publisher<T> readTransaction(RxTransactionWork<? extends Publisher<T>> rxTransactionWork) {
    ...
    return Mono.from(rxSession.readTransaction(new TracingRxTransactionWork<>(rxTransactionWork, span, tracer)))
            .doOnSuccess(t -> span.finish())
            .doOnError(throwable -> {
  ...
  public <T> Publisher<T> readTransaction(RxTransactionWork<? extends Publisher<T>> rxTransactionWork, TransactionConfig transactionConfig) {
    ...
    return Mono.from(rxSession.readTransaction(new TracingRxTransactionWork<>(rxTransactionWork, span, tracer), transactionConfig))
            .doOnSuccess(t -> span.finish())
            .doOnError(throwable -> {
              onError(throwable, span);
              span.finish();
  ...
  public <T> Publisher<T> writeTransaction(RxTransactionWork<? extends Publisher<T>> rxTransactionWork) {
    ...
    return Mono.from(rxSession.writeTransaction(new TracingRxTransactionWork<>(rxTransactionWork, span, tracer)))
            .doOnSuccess(t -> span.finish())
            .doOnError(throwable -> {
              onError(throwable, span);
              span.finish();
  ...
  public <T> Publisher<T> writeTransaction(RxTransactionWork<? extends Publisher<T>> rxTransactionWork, TransactionConfig transactionConfig) {
    ...
    return Mono.from(rxSession.writeTransaction(new TracingRxTransactionWork<>(rxTransactionWork, span, tracer), transactionConfig))
            .doOnSuccess(t -> span.finish())
            .doOnError(throwable -> {
              onError(throwable, span);
              span.finish();
